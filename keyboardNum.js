...
var keyBoardNum = function(numberId,option){
    var me = this,insertId = '#keyBoardNum';
    var defaultOpt = {
    ...
    var html = '<div id="keyBoardNum">'+
                ...
                '    </i>'+
                '    <i data-num="callback" id="keyBoardPay">'+me.option.btnText+'</i>'+
                '    <i data-num="4">4</i>'+
                '    <i data-num="5">5</i>'+
...
keyBoardNum.prototype.delete = function(dom){
    ...
    if(str !== ''){
        ...
        if(dom.text()==''){
            $('#keyBoardPay').removeClass('active');
            this.num = 0;
        }else this.num = dom.text();
    }
}
keyBoardNum.prototype.callback = function(){
    if($('#keyBoardPay').hasClass('active')){
        this.option.btnCallBack && this.option.btnCallBack(parseFloat(this.num).toFixed(this.option.decimal));
    }
}
...
keyBoardNum.prototype.insert = function(dom,num,option){
    ...
    this.num = str + num;
    dom.text(this.num);
    $('#keyBoardPay').addClass('active');
}

function checkOption(val,defaultVal){
